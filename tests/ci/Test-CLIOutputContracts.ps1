...
Check 'stage labels use compact slash form' ($startHere.Contains('STEP 1/9: Tool pack + KVRT/ESET staging (Malwarebytes fallback at Step 6)') -and $startHere.Contains('STEP 9/9: Report') -and -not $startHere.Contains('/10:'))
Check 'no read-only claim remains on a removing tool' (-not $startHere.Contains('read-only') -and -not $startHere.Contains('readonly'))
Check 'steps 1-2 run automatically with no decision prompt' (-not $startHere.Contains('STEP 1/9') -or ($startHere.Contains('STEP 1/9: Tool pack + KVRT/ESET staging (Malwarebytes fallback at Step 6)') -and -not $startHere.Contains('set /p GO="    Run now? [Y/n] "')))
Check 'destructive removal warning remains' ($startHere.Contains('quarantined, never deleted'))
Check 'guided step 5 delegates to the single-confirmation review' ($startHere.Contains('Invoke-ReviewAndRemove.ps1') -and $startHere.Contains('STEP 5/9: Review/remove ScreenConnect'))
Check 'cleanup stage 3 asks one remove-all confirmation' ($cleanup.Contains('Remove all detected ScreenConnect instances?'))
Check 'AV never-silent promise remains' ($startHere.Contains('Never silent-uninstalls - uninstallers open for you to drive.'))
Check 'attended launch prompts remain' ($startHere.Contains('Launch KVRT? [Y/n]') -and -not $startHere.Contains('Run Tikun now? [y/N]'))
Check 'Tikun is removed from the guided runner' (-not $startHere.Contains('Tikun') -and -not $startHere.Contains('GeneralFix'))
Check 'filter-block diagnostic path remains' ($scanner.Contains('DiagnosticsOnly') -and $startHere.Contains('scanner-Malwarebytes-result.json'))
