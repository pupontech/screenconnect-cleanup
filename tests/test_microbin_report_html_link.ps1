...
function Start-MicroBinServer {
    ...
    $port = $null
    # A cold interpreter start on a loaded Windows runner can take longer than
    # the old 5s cap, which produced a false 'receiver did not start' failure.
    # Wait on a deadline instead, and stop early when the launcher is already
    # gone so a broken interpreter fails fast with its own diagnostics.
    ...
    if ($null -eq $port) {
        ...
        [void]$details.AppendLine('port file not written: ' + $portFile)
        if ($proc.HasExited) {
