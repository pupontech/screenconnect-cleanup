...
if ($kvrt.LogPath -eq $kvrtReport) { Write-Host "PASS  KVRT report path recorded" } else { Write-Host ("FAIL  KVRT report path recorded: got [" + $kvrt.LogPath + "] expected [" + $kvrtReport + "]"); $script:failures++ }
