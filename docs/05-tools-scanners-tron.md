...
| **KVRT** | Kaspersky removal tool | Staged from Kaspersky's official current KVRT download URL and launched attended via `Invoke-GUIScanner.ps1` (`-Scanner KVRT`). No silent scan/clean flags are passed. The only launch flags are the report-format trio `-accepteula -dontencrypt -details` (vendor doc: support.kaspersky.com/kvrt2024/269475): the GUI stays attended and reports land as plain text under `C:\KVRT_Data\Reports` (or `C:\KVRT2020_Data\Reports`), which `Get-ScannerFindings.ps1` parses for the HTML report. `-silent` and `-processlevel` are never passed (`-processlevel` would auto-neutralize detected objects). | **APPROVED by the owner (decision D2).** Do not block on licensing. |
...
| **Malwarebytes** | Install via winget (`winget install -e --id Malwarebytes.Malwarebytes --accept-package-agreements --accept-source-agreements`); when winget is missing, fails, or installs without a usable GUI, `Invoke-GUIScanner.ps1` downloads the official Malwarebytes consumer offline installer (`https://downloads.malwarebytes.com/file/mb5_offline`), validates it, launches it visibly, then launches and waits on `mbam.exe`; uninstall remains via winget with the vendor-uninstaller fallback. | Paid, per-endpoint. Not approved for automation; the visible GUI remains technician-attended. A failed winget attempt still records read-only endpoint/DNS/proxy/hosts diagnostics before the offline fallback. |
...
Realistic Stage 5 line-up (owner update): **KVRT + ESET Online Scanner** attended GUI launches + **Malwarebytes via winget with the official offline-installer fallback** (install/uninstall).
